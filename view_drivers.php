...
<body>
    <header style="background-color: #2c3e50; color: white; padding: 15px 0;">
        <div class="container">
            <h1><i class="fa-solid fa-car"></i> Registered Drivers</h1>
    ...
    <main>
        <div class="container">
            <form method="GET" class="search-box">
                <input type="text" name="search" placeholder="Search by Name, Email, or License ID" value="<?php echo htmlspecialchars($search); ?>">
                <button type="submit"><i class="fa-solid fa-search"></i> Search</button>
                <a href="view_drivers.php" style="padding: 8px 15px; background: #7f8c8d; color: white; text-decoration: none;">Reset</a>
            </form>
                    ...
                    <?php if (mysqli_num_rows($result) > 0): ?>
                        <?php while($row = mysqli_fetch_assoc($result)): ?>
                            <tr>
                                ...
                                <td style="font-weight:bold; color: <?php echo ($row['verification_status']=='verified')?'green':'red'; ?>">
                                    <?php echo ucfirst($row['verification_status']); ?>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="7" style="text-align:center;">No drivers found.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
