...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        setInterval(() => {
            fetch('admin_check_notifications.php').then(response => response.json()).then(data => {
                if (data.unread_count > lastUnreadCount) {
                    ...
                    let link = data.driver_unread > 0 ? 'admin_driver_chat.php' : 'admin_student_chat.php';
                    Swal.fire({
                        position: 'top-end', icon: 'info', title: 'New Message',
                        text: 'You have a new support message.', showConfirmButton: true,
                        confirmButtonText: 'View', toast: true, timer: 5000
