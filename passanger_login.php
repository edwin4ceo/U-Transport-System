...
// =========================================================
// FUNCTION: CHECK LOGIN STATUS (FIXED LOGIC)
// =========================================================
...
<style>
    ...
    .top h2 { font-size: 32px; color: #333 !important; font-weight: 600; margin: 0; padding: 0; background: none !important; box-shadow: none !important; }
    
    .input-box { display: flex; align-items: center; width: 100%; height: 55px; background: #ffffff !important; box-shadow: 0 5px 15px rgba(0,0,0,0.05) !important; border-radius: 30px !important; margin-bottom: 20px; padding: 0 20px; border: 2px solid #c4c4c4 !important; transition: .3s; }
    .input-box:focus-within { background: #ffffff !important; box-shadow: 0 4px 10px rgba(0, 90, 156, 0.15) !important; border: 2px solid #005A9C !important; }
    ...
    .gender-option label { display: flex; align-items: center; justify-content: center; gap: 10px; width: 100%; height: 55px; background: #ffffff; border: 2px solid #c4c4c4; border-radius: 30px; font-size: 15px; color: #555; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 5px 15px rgba(0,0,0,0.05); }
    label[for="gender-male"]:hover { background: #f0f8ff; border-color: #005A9C; }
    #gender-male:checked + label { background: #005A9C; color: #ffffff; border-color: #005A9C; box-shadow: 0 4px 10px rgba(0, 90, 156, 0.3); transform: translateY(-2px); }
    label[for="gender-female"]:hover { background: #fff0f5; border-color: #E91E63; }
    #gender-female:checked + label { background: #E91E63; color: #ffffff; border-color: #E91E63; box-shadow: 0 4px 10px rgba(233, 30, 99, 0.3); transform: translateY(-2px); }
    
    input:-webkit-autofill { -webkit-box-shadow: 0 0 0 30px white inset !important; -webkit-text-fill-color: #333 !important; }
    .input-field { flex: 1; background: transparent !important; border: none !important; outline: none !important; color: #333 !important; font-size: 15px !important; height: 100%; padding: 0 !important; margin: 0 !important; box-shadow: none !important; }
    .toggle-pass { margin-right: 0; margin-left: 10px; cursor: pointer; color: #999; }
    .toggle-pass:hover { color: #005A9C; }
    
    .submit { width: 100%; height: 55px; background: #005A9C !important; border: none !important; border-radius: 30px !important; color: #fff !important; font-size: 16px; font-weight: 600; cursor: pointer; transition: .3s; box-shadow: 0 8px 15px rgba(0, 90, 156, 0.2); margin-top: 10px; display: flex; align-items: center; justify-content: center; }
    .submit:hover { background: #004a80 !important; transform: translateY(-2px); box-shadow: 0 10px 20px rgba(0, 90, 156, 0.3); }
    .submit:disabled { background: #ccc !important; cursor: not-allowed; }
...
<div class="wrapper">
    <div class="back-nav"><a href="index.php" class="btn-back"><i class="fa-solid fa-arrow-left"></i> Back</a></div>
    ...
    <div class="form-box">
        <div class="login-container" id="login">
            <div class="top"><span>Don't have an account? <a href="#" onclick="register()">Sign Up</a></span><h2>Login</h2></div>
            <form action="" method="POST" onsubmit="handleLoading(this)">
                <input type="hidden" name="action" value="login">
                <div class="input-box"><i class="fa-regular fa-envelope"></i><input type="email" name="login_email" class="input-field" placeholder="Username or Email" value="<?php echo htmlspecialchars($login_email_val); ?>" required></div>
                <div class="input-box"><i class="fa-solid fa-lock"></i><input type="password" name="login_password" id="loginPass" class="input-field" placeholder="Password" required><i class="fa-solid fa-eye-slash toggle-pass" onclick="togglePass('loginPass', this)"></i></div>
                <button type="submit" name="login" class="submit">Sign In</button>
                <div class="two-col"><div class="two"><a href="passanger_forgot_password.php">Forgot password?</a></div></div>
            </form>
        </div>

        <div class="register-container" id="register">
            <div class="top"><span>Have an account? <a href="#" onclick="login()">Login</a></span><h2>Sign Up</h2></div>
            <form action="" method="POST" onsubmit="handleLoading(this)">
                <input type="hidden" name="action" value="register">
                <div class="input-box"><i class="fa-regular fa-user"></i><input type="text" name="reg_name" class="input-field" value="<?php echo htmlspecialchars($reg_name); ?>" placeholder="Full Name" style="text-transform: uppercase;" oninput="this.value = this.value.toUpperCase()" required></div>
                <div class="input-box"><i class="fa-solid fa-id-card"></i><input type="text" name="reg_student_id" id="studentIDInput" class="input-field" value="<?php echo htmlspecialchars($reg_student_id); ?>" placeholder="Student ID" required></div>
                <div class="input-box"><i class="fa-solid fa-envelope"></i><input type="email" name="reg_email" id="emailInput" class="input-field" value="<?php echo htmlspecialchars($reg_email); ?>" placeholder="ID@student.mmu.edu.my" required></div>
                <div class="gender-box">
                    <div class="gender-option"><input type="radio" name="reg_gender" id="gender-male" value="Male" <?php echo ($reg_gender == 'Male') ? 'checked' : ''; ?>><label for="gender-male"><i class="fa-solid fa-mars"></i> Male</label></div>
                    <div class="gender-option"><input type="radio" name="reg_gender" id="gender-female" value="Female" <?php echo ($reg_gender == 'Female') ? 'checked' : ''; ?>><label for="gender-female"><i class="fa-solid fa-venus"></i> Female</label></div>
                </div>
                <div class="input-box"><i class="fa-solid fa-lock"></i><input type="password" name="reg_password" id="regPass" class="input-field" placeholder="Password (Min 6 chars)" minlength="6" required><i class="fa-solid fa-eye-slash toggle-pass" onclick="togglePass('regPass', this)"></i></div>
                <div class="input-box"><i class="fa-solid fa-lock"></i><input type="password" name="reg_confirm_password" id="regConfPass" class="input-field" placeholder="Confirm Password" required><i class="fa-solid fa-eye-slash toggle-pass" onclick="togglePass('regConfPass', this)"></i></div>
...
<script>
    ...
    function login() { x.style.left = "0px"; y.style.right = "-120%"; x.style.opacity = 1; x.style.pointerEvents = "auto"; y.style.opacity = 0; y.style.pointerEvents = "none"; a.className += " white-btn"; b.className = "btn"; }
    function register() { x.style.left = "-120%"; y.style.right = "0px"; x.style.opacity = 0; x.style.pointerEvents = "none"; y.style.opacity = 1; y.style.pointerEvents = "auto"; a.className = "btn"; b.className += " white-btn"; }
    function togglePass(inputId, icon) { const input = document.getElementById(inputId); if (input.type === "password") { input.type = "text"; icon.classList.replace('fa-eye-slash', 'fa-eye'); } else { input.type = "password"; icon.classList.replace('fa-eye', 'fa-eye-slash'); } }
    function handleLoading(form) { const btn = form.querySelector('button[type="submit"]'); const originalText = btn.innerHTML; if(btn.disabled) return false; btn.disabled = true; btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Processing...'; setTimeout(() => { btn.disabled = false; btn.innerHTML = originalText; }, 10000); return true; }
</script>
