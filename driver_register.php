...
if (isset($_POST['register'])) {
    ...
    // 2. Basic validation (Ensure gender is selected)
    if (
        $full_name === "" ||
        $identification_id === "" ||
        $email === "" ||
        $phone_number === "" ||    
        $gender === "" ||          // [Check] Gender cannot be empty
        $license_number === "" ||  
        $license_expiry === "" ||  
        $password_plain === "" ||
        $car_model === "" ||
        ...
        $_SESSION['swal_msg']   = "Please fill in all required fields, including gender and vehicle details.";
    ...
    // 4. Insert into drivers table (Added Gender)
    // NOTE: Ensure your database 'drivers' table has a 'gender' column.
