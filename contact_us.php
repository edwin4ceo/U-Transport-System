...
// [FIX] Correct PHPMailer paths based on your folder structure (No /src/)
...
// Identify user role (Guest, Driver, or Student)
$is_guest = true;
$current_role = '';
$current_id = '';
...
if ($is_guest && isset($_POST['send_email'])) {
    ...
    try {
        ...
        $_SESSION['swal_msg'] = "Your message has been sent to our support team.";
...
<style>
    ...
    /* 2. Navigation Back Button (Fixed Top Left) */
    .btn-back-nav {
    ...
    .email-sky-card {
        ...
        width: 100%; box-sizing: border-box; transition: 0.3s ease;
    }
    ...
    .expanding-pill {
        ...
        overflow: hidden; text-decoration: none; transition: width 0.5s cubic-bezier(0.4, 0, 0.2, 1), background 0.3s;
    ...
    .btn-pill-action {
        ...
        font-size: 15px !important; font-weight: 600 !important; cursor: pointer; transition: 0.3s;
        box-shadow: 0 4px 15px rgba(0, 75, 130, 0.2);
    }
    .btn-pill-action:hover { background-color: #003660 !important; transform: translateY(-2px); }
    ...
    <div class="contact-container">
        ...
        <div class="interaction-panel">
            <?php if ($is_guest): ?>
                ...
                <form method="POST">
                    <div class="field-group"><label>Full Name</label><input type="text" name="name" class="field-input" placeholder="Your Name" required></div>
                    <div class="field-group"><label>Email Address</label><input type="email" name="email" class="field-input" placeholder="example@mail.com" required></div>
                    <div class="field-group"><label>Message</label><textarea name="message" class="field-input" placeholder="How can we assist you today?" required></textarea></div>
                    <div class="btn-submit-container"><button type="submit" name="send_email" class="btn-pill-action">Send Message</button></div>
                </form>
            <?php else: ?>
...
<script>
    Swal.fire({ title: '<?php echo $_SESSION['swal_title']; ?>', text: '<?php echo $_SESSION['swal_msg']; ?>', icon: '<?php echo $_SESSION['swal_type']; ?>', confirmButtonColor: '#004b82' });
</script>
