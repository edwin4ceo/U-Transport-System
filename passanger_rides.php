...
// ==========================================
// SECTION 2: HANDLE ACTIONS (CANCEL / RATE / FAVOURITE)
// ==========================================
...
if(isset($_POST['submit_rating'])){
    ...
    if($check->num_rows > 0){
        ...
        $sql = "INSERT INTO reviews (booking_id, student_id, rating, comment, created_at) VALUES ('$b_id', '$student_id', '$rating', '$comment', NOW())";
    }
...
// 3. Handle Favourite Toggle (AJAX)
...
<style>
    ...
    /* Modal Styling */
    .modal-overlay { display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0, 0, 0, 0.5); backdrop-filter: blur(4px); z-index: 9999 !important; align-items: center !important; justify-content: center !important; }
    ...
    <div id="content-history" style="display:none;">
        ...
        <div class="ride-card">
            ...
            <div class="card-content-row">
                ...
                <div class="info-right-col">
                    <span class="status-pill <?php echo ($status == 'COMPLETED') ? 'st-completed' : 'st-cancelled'; ?>"><?php echo $status; ?></span>
                    <?php if(!empty($row['driver_name'])): ?>
                        <div class="driver-box-design" onclick="openDriverModal(this)" 
                             data-did="<?php echo $row['driver_id']; ?>"
                             data-name="<?php echo htmlspecialchars($row['driver_name']); ?>" 
...
<div id="driverModal" class="modal-overlay" onclick="closeDriverModal(event)">
    <div class="modal-content" onclick="event.stopPropagation()">
        ...
        <div style="display: flex; gap: 10px; margin-top: 20px;">
            <button id="m_fav_btn" class="btn-pill-auto" style="flex: 1; border-radius: 50px !important;" onclick="toggleFavModal()">
                <i id="m_fav_icon" class="fa-solid fa-heart" style="margin-right: 8px !important;"></i> 
                <span id="m_fav_text">Save</span>
            </button>
...
<script>
    <?php if(isset($_SESSION['cancel_success'])): ?>
        Swal.fire({ title: 'Cancel Successful!', text: 'Your ride booking has been cancelled.', icon: 'success', confirmButtonColor: '#004b82' });
    ...
    function openDriverModal(el) {
        ...
        fd.append('check_fav', '1');
        fd.append('driver_id', currentDriverIdModal);
    ...
    function updateFavBtnUI(isFav) {
        ...
        if(isFav) {
            btn.className = "btn-pill-auto style-saved-modal";
            text.innerText = "Saved";
        } else {
            btn.className = "btn-pill-auto style-save-add-modal";
            text.innerText = "Save";
        }
    }
    ...
    function toggleFavModal() {
        ...
        // FIX: 指定 target 为 driverModal 确保确认框跳在最前面
        Swal.fire({
            target: document.getElementById('driverModal'),
            title: isSaving ? 'Save Driver?' : 'Unsave Driver?',
            text: isSaving ? `Do you want to save ${currentDriverNameModal}?` : `Remove ${currentDriverNameModal}?`,
            icon: isSaving ? 'question' : 'warning',
            showCancelButton: true,
            confirmButtonColor: isSaving ? '#004b82' : '#e11d48',
            confirmButtonText: isSaving ? 'Yes, save' : 'Yes, remove',
            reverseButtons: true
        }).then((result) => {
            if (result.isConfirmed) {
                ...
                fetch('passanger_rides.php', { method: 'POST', body: fd }).then(res => res.json()).then(data => {
                    ...
                    Swal.fire({ 
                        target: document.getElementById('driverModal'),
                        title: 'Success!', 
                        text: data.status === 'added' ? 'Driver saved!' : 'Removed!', 
                        icon: 'success', 
                        timer: 1000, 
    ...
    function handleRating(bookingId, currentRating, currentComment) {
        if(currentRating && currentRating !== "") {
            ...
            Swal.fire({
                title: 'Your Review',
                html: `<div style="margin-bottom: 15px;">${starsHtml}</div><p style="background: #f8fafc; padding: 15px; border-radius: 12px; font-style: italic; color: #475569; border: 1px solid #e2e8f0;">"${currentComment || 'No comment provided.'}"</p>`,
                showCancelButton: true, confirmButtonText: 'Edit Review', cancelButtonText: 'Close', confirmButtonColor: '#f97316', cancelButtonColor: '#64748b'
            }).then((result) => { if (result.isConfirmed) showRatePopup(bookingId, currentRating, currentComment, true); });
        } else { showRatePopup(bookingId, 0, "", false); }
    }

    function showRatePopup(bookingId, r, c, isEdit) {
        Swal.fire({
            title: isEdit ? 'Edit Review' : 'Rate & Review',
            html: `<div style="font-size: 28px; margin-bottom: 20px;">${[1,2,3,4,5].map(n => `<i class="fa-star ${n <= r ? 'fa-solid' : 'fa-regular'} star-btn" data-val="${n}" style="cursor:pointer; color:#f97316; margin: 0 5px;"></i>`).join('')}</div><input type="hidden" id="star_val" value="${r || 0}"><textarea id="swal_cmt" class="swal2-textarea" placeholder="Describe your experience..." style="width: 85%; height: 100px; font-size: 14px;">${c || ''}</textarea>`,
            didOpen: () => {
            ...
            showCancelButton: true, confirmButtonText: 'Submit', confirmButtonColor: '#004b82',
            preConfirm: () => {
            ...
            if (result.isConfirmed) {
                if (isEdit) {
                    Swal.fire({ title: 'Are you sure?', text: "Do you want to change your review?", icon: 'warning', showCancelButton: true, confirmButtonColor: '#004b82', confirmButtonText: 'Yes, update it!' }).then((confirmResult) => { if (confirmResult.isConfirmed) submitReviewAjax(bookingId, result.value.rating, result.value.comment, true); });
                } else { submitReviewAjax(bookingId, result.value.rating, result.value.comment, false); }
            }
        });
    }

    function submitReviewAjax(bookingId, rating, comment, isUpdate) {
        const fd = new FormData();
        fd.append('submit_rating', '1'); fd.append('booking_id', bookingId); fd.append('rating', rating); fd.append('comment', comment);
        fetch('passanger_rides.php', { method: 'POST', body: fd }).then(res => res.json()).then(data => {
            if(data.status === 'success') {
                Swal.fire({ title: isUpdate ? 'Updated!' : 'Submitted!', text: isUpdate ? 'Review changed.' : 'Thank you!', icon: 'success', confirmButtonColor: '#004b82' }).then(() => location.reload());
            }
        });
    }

    function switchTab(tabName) {
        document.getElementById('content-ongoing').style.display = (tabName === 'ongoing') ? 'block' : 'none';
        document.getElementById('content-history').style.display = (tabName === 'history') ? 'block' : 'none';
        document.getElementById('btn-ongoing').classList.toggle('active', tabName === 'ongoing');
        document.getElementById('btn-history').classList.toggle('active', tabName === 'history');
    }
</script>
